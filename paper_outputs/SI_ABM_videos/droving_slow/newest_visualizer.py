...
#for t in range(0,100):
    if t%modder == 0:
        ...
        title = 'Timestepping: ' + str(int(dat_times[index]))
        fig.suptitle(title)
        ...
        if clone_view == True:
            ...
            #plot the colorwheel angle labels
            angle_label_angles = np.arange(0, 2*np.pi, 2*np.pi/len(angle_labels))
            angle_locs_x = (scale/3)*np.cos(angle_label_angles)
            angle_locs_y = (scale/3)*np.sin(angle_label_angles)
            angle_locs_x_rot = angle_locs_x*np.cos(tmp_herd_dog_angle)-angle_locs_y*np.sin(tmp_herd_dog_angle)+x_offset-scale/15
            angle_locs_y_rot = angle_locs_y*np.cos(tmp_herd_dog_angle)+angle_locs_x*np.sin(tmp_herd_dog_angle)+y_offset
            for kk in range(len(angle_labels)):
                ax1.text(angle_locs_x_rot[kk], angle_locs_y_rot[kk], angle_labels[kk])
        ...
        if drone_view == True:
            ...
            #plot the colorwheel angle labels
            angle_label_angles = np.arange(0, 2*np.pi, 2*np.pi/len(angle_labels))
            angle_locs_x = 1.1*np.cos(angle_label_angles)
            angle_locs_y = 1.1*np.sin(angle_label_angles)
            angle_locs_x_rot = angle_locs_x*np.cos(tmp_herd_dog_angle)-angle_locs_y*np.sin(tmp_herd_dog_angle)+x_offset-0.1
            angle_locs_y_rot = angle_locs_y*np.cos(tmp_herd_dog_angle)+angle_locs_x*np.sin(tmp_herd_dog_angle)+y_offset
            for kk in range(len(angle_labels)):
                ax2.text(angle_locs_x_rot[kk], angle_locs_y_rot[kk], angle_labels[kk])
