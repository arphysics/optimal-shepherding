...
def plot_herd_snapshot(temp_H, curr_time, temp_counterr, loc_arrays, traj_array = [0,0,0]):
  ...
  title = 'Timestepping:' + str(curr_time)
  fig.suptitle(title)
  plot_name = './test_plots/test_plot'+str(temp_counterr).zfill(4)


  #plot from the Dog Frame
  ax1.plot(phi_array_sum+theta, r_array_sum, '.', label = 'Herd Boundary')
  ax1.plot(0,0, 'ro', markersize = 10, label = 'Shepherd')
  ax1.plot(np.ones(20)*np.pi*3/4, np.linspace(0, max_plot_radius, 20), 'g--', label = "Goal Angle")
  ax1.set_ylim(0, max_plot_radius) #only use this one for polar
  ax1.set_yticks([max_plot_radius/4, max_plot_radius/2, max_plot_radius])
  #ax1.plot(, 'gD', markersize = 20, label = "Target")
  ax1.legend(loc='upper right')



  #Plot from the herd frame
  ax2.plot(x_rot+x_shift, y_rot+y_shift, '.', label = 'Herd Boundary')
  ax2.plot(x_dog, y_dog, 'ro', markersize = 10, label = 'Shepherd')
  ...
  ax2.pcolormesh(xd, yd, zd, shading='gouraud', cmap='Greys', label = 'Trajectory', alpha = 0.2)
  ax2.plot(-15.0, 15.0, 'gD', markersize = 20)
